...
(async()=>{
 ...
 await check('A1 fresh home and lesson initialization',async p=>{
  ...
  await p.getByRole('button',{name:'下一部分',exact:true}).click(); await active(p,'s-mechanism');
  assert.equal((await state(p)).lastLocation.stepId,'s-mechanism');
  await p.goto(url+'index.html'); assert.equal(await p.locator('#home-primary').innerText(),'继续学习');
  assert.match(await p.locator('#home-primary').getAttribute('href'),/#s-mechanism$/);
 });
 ...
 await check('A7 current catalog progress and 10-item review pagination',async p=>{
  ...
  assert.equal(items.length,32); const s=fresh();for(const q of items)s.questions[q.id]={revision:q.revision,attempts:1,wrongAttempts:1,lastOptionId:'a',lastCorrect:false,needsReview:true};
  await seed(p,s);await p.goto(url+'review.html');assert.equal(await p.locator('#review-list li').count(),10);
  for(let i=0;i<3;i++)await p.getByRole('button',{name:'下一页',exact:true}).click();assert.equal(await p.locator('#review-list li').count(),2);
  await p.goto(url+'course.html');const lids=await p.locator('li[data-lesson-id]').evaluateAll(ns=>ns.map(n=>n.dataset.lessonId));
  for(const lid of lids)s.lessons[lid]={completed:true};await seed(p,s);await p.reload();assert.equal(await p.locator('#stat-read').innerText(),'16');assert.equal(await p.locator('#home-primary').innerText(),'重新查看课程');
 });
