...
def main(argv=None):
    ...
    def tally(ls):
        return {'lessons':len(ls),'published':sum(l['status']=='published' for l in ls),
                'quizzes':sum(s['type']=='quiz' for l in ls for s in l['steps']),
                'practices':sum(s['type']=='practice' for l in ls for s in l['steps'])}
    core=[l for l in lessons if not l['id'].startswith('m0-')]
    m0=[l for l in lessons if l['id'].startswith('m0-')]
    report['inventory']={'core':tally(core),'m0':tally(m0),
    ...
    if report['inventory']['core']!={'lessons':16,'published':16,'quizzes':32,'practices':4}:
        report['errors'].append(f"Core 口径验收失败: {report['inventory']['core']}")
    if report['inventory']['m0']!={'lessons':3,'published':3,'quizzes':6,'practices':1}:
        report['errors'].append(f"M0 口径验收失败: {report['inventory']['m0']}")
