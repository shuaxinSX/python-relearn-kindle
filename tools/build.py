...
def main(argv=None) -> int:
    ...
    # lesson -> module 映射（序号显示：m0 模块用 M0-1/M0-2/M0-3，其余模块用核心计数 1-16）
    lesson_module = {}
    for module in model["modules"]:
        for lesson_id in module["lessonIds"]:
            lesson_module[lesson_id] = module["id"]
        ...
        for lesson_id in module["lessonIds"]:
            if lesson_id in lessons_ctx:
                if module["id"] == "m0":
                    ...
                    seq += 1
                    label = seq
                items.append({**lessons_ctx[lesson_id], "seq": label})
        if items:
    ...
    for lesson in all_lessons:
        if lesson_module.get(lesson["id"]) == "m0":
            m0_seq += 1
            lesson["seq"] = f"M0-{m0_seq}"
        else:
            seq += 1
            lesson["seq"] = seq
